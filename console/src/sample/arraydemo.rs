...
use crate::console::utils::display_transaction_receipt;
...
pub fn demo(cli: &Cli) {
    ...
    let mut bcossdk = Bcos2Client::new_from_config(configfile.as_str()).unwrap();
    ...
    let res = bcossdk
        .sendRawTransactionGetReceipt(&contract, &to_address, "add", params.as_slice())
        .unwrap();
