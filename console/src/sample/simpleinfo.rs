...
use crate::console;
use rust_gears_sdk::bcos2sdk::bcos2client;
...
pub fn demo_deploy_simpleinfo(bcossdk: &mut Bcos2Client) -> Result<String, KissError> {
    ...
    console::utils::display_transaction_receipt(&receipt, &Option::from(None), &bcossdk.config);
...
pub fn demo_simpleinfo_set(
    ...
    console::utils::display_transaction_receipt(&res, &Option::from(contract), &bcossdk.config);
...
pub fn demo(configfile: &str) {
    ...
    let callvalue = bcossdk
        .call(&contract, &to_address, "getall", &["".to_string()])
        .unwrap();
