...
use rust_gears_sdk::bcos2sdk::bcossdkquery;
...
use crate::cmdmap;
use crate::common::{Cli, OptContract};
use crate::console::cmdmap::CliCmdMap;
use crate::console::compile::console_compile;
use crate::sample::demo_bcos3client::demo_bcos3client;
use rust_gears_sdk::bcos2sdk::bcossdkquery::json_hextoint;
use rust_gears_sdk::bcossdkutil::bcosclientconfig::{BcosCryptoKind, ClientConfig};
...
pub fn deploy(cli: &Cli) -> Result<(), KissError> {
    ...
    let txhash = match res {
        Ok(v) => {
            let hash = String::from(v["result"].as_str().unwrap());
            hash
        }
        Err(e) => return Err(e),
    };
...
pub fn sendtx(cli: &Cli) -> Result<(), KissError> {
    ...
    let response = bcossdk.send_raw_transaction(
        &contract,
        address.as_str(),
        opt.method.as_str(),
        opt.params.as_slice(),
    ...
    let receiptresult = bcossdk.try_getTransactionReceipt(txhash, 3, false);
    //println!("receiptresult result :{:?} ",receiptresult);
    match receiptresult {
        Ok(receipt) => {
            crate::console::utils::display_transaction_receipt(
                &receipt,
